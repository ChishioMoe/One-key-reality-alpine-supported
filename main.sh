...
# 4. 安装 Xray
# ====================================================
echo -e "${yellow}安装 Xray 核心组件...$none"
...
# 网络栈选择
if [[ -z $netstack ]]; then
    read -p "$(echo -e "如果是双栈，请选择网口 (输入 ${cyan}4${none} 为 IPv4, ${cyan}6${none} 为 IPv6，默认4): ") " netstack
    if [[ $netstack == "6" ]]; then ip=${IPv6}; else ip=${IPv4}; netstack=4; fi
fi
...
# UUID 设置
if [[ -z $uuid ]]; then
    read -p "$(echo -e "请输入 UUID (默认: ${cyan}${default_uuid}$none): ")" uuid
    uuid=${uuid:-$default_uuid}
fi

# Reality 密钥生成
reality_key_seed=$(echo -n ${uuid} | md5sum | head -c 32 | base64 | tr '+/' '-_' | tr -d '=')
...
cat > /usr/local/etc/xray/config.json <<-EOF
{
  ...
  "outbounds": [
    ...
    { "protocol": "socks", "settings": { "servers": [{ "address": "127.0.0.1", "port": 40000 }] }, "tag": "socks5-warp" },
...
# ====================================================
if [[ $netstack == "6" ]]; then
    warn "IPv6 小鸡，尝试添加 IPv4 WARP 出站..."
    curl -LO https://gitlab.com/fscarmen/warp/-/raw/main/menu.sh && yes "" | bash menu.sh 4
    restart_xray
elif [[ $netstack == "4" ]]; then
    warn "IPv4 小鸡，尝试添加 IPv6 WARP 出站..."
    curl -LO https://gitlab.com/fscarmen/warp/-/raw/main/menu.sh && yes "" | bash menu.sh 6
    restart_xray
fi

echo -e "$green节点信息已保存至 ~/_vless_reality_url_$none"
